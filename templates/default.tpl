      ...
      {foreach from=$contacts key=id item=contact}
        <td>
          {if $contact.contact_type == 'Organization'}
            <p style="margin: 5px;">Organization Name:
              {if $contact.organization_name}{$contact.organization_name}{else}N/A{/if}</p>
          {elseif $contact.contact_type == 'Individual'}
            <p style="margin: 5px;">Name: {if @$contact.prefix}{$contact.prefix}
                {/if}{if $contact.first_name}{$contact.first_name} {/if}{if $contact.last_name}{$contact.last_name}{/if}</p>
            {/if}
            <p style="margin: 5px;">Address 1: {if $contact.street_address}{$contact.street_address}{else}N/A{/if}</p>
            <p style="margin: 5px;">Address 2:
              {if $contact.supplemental_address_1}{$contact.supplemental_address_1}{else}N/A{/if}</p>
